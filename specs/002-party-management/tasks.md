...
- [ ] T022 [US4] 관리자 상세 제어 페이지(`admin/parties/[id]`) 및 수정 API 구축
    - [ ] 파티 시간(시작/종료) 및 정원(인원수) 실시간 수정 기능 구현
    - [ ] **초기 상호작용 횟수 3종 프리셋(일반/열정/커스텀) 설정 기능 구현**
    - [ ] 닉네임 등록자 중 "2차 신청 미완료자" 필터링 뷰 구현
    - [ ] 3_prj_docs/완료내역/TASK_T022_완료내역.md 작성
- [ ] T023 [US5] 실시간 통합 모니터링 피드 및 닉네임 로그 뷰어 구현
    - [ ] 모든 상호작용(쪽지, 호감도, SOS) 실시간 스트리밍 대시보드 구축
    - [ ] 참여자별 닉네임 변경 전후 이력 추적 및 조회 기능 구현
    - [ ] 3_prj_docs/완료내역/TASK_T023_완료내역.md 작성
- [ ] T024 [US4] 참여자 신청 상태 관리 및 상호작용 잔여 횟수(개수) 조정 기능
    - [ ] 관리자 UI에서 1차/2차 신청 상태 토글 및 승인/반려 기능 구현
    - [ ] 개별 참여자별 큐피트/호감도 잔여 횟수 실시간 가감(개수 조정) 기능 구현
    - [ ] **전체 참여자 대상 횟수 일괄 지급 기능(Bulk Grant) 구현**
    - [ ] 3_prj_docs/완료내역/TASK_T024_완료내역.md 작성
- [ ] T025 [US4] 참여자 대시보드 UX 고도화 (타이머 & 보관함)
    - [ ] 참여자 화면 상단 실시간 파티 종료 카운트다운 타이머 구현
    - [ ] 수신/발신 상세 내역 조회 및 알림 이력 보관함(`inbox`) 구현
    - [ ] 3_prj_docs/완료내역/TASK_T025_완료내역.md 작성
- [ ] T026 [US5] 랭킹 페이지 UI 고도화 및 본인 순위 강조
    - [ ] 랭킹 리스트 내 참여자 본인 위치 시각적 강조(색상, 강조선 등) 로직 적용
    - [ ] 1,000명 이상 데이터 상황에서의 랭킹 리스트 렌더링 성능 최적화
    - [ ] 3_prj_docs/완료내역/TASK_T026_완료내역.md 작성
